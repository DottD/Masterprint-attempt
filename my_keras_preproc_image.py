...
def load_img(path, grayscale=False, target_size=None,
						 interpolation='nearest', crop_shape=None):
		...
		# Arguments
				...
				crop_shape: if the loaded image is large enough a random cropping is performed
		...
		if grayscale:
				...
				if img.mode != 'RGB':
						img = img.convert('RGB')
		if crop_shape and all([img.size[i] >= crop_shape[i] for i in range(2)]):
			x, y = [np.random.randint(0, img.size[i]-crop_shape[i]+1) for i in range(2)]
			box=(x, y, x+crop_shape[0], y+crop_shape[1])
